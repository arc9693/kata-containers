...
func (k *kataAgent) createContainer(ctx context.Context, sandbox *Sandbox, c *Container) (p *Process, err error) {
	...
	cifsStorages, err := k.handleSMBMounts(c, ociSpec)
	if err != nil {
		return nil, err
	}
	ctrStorages = append(ctrStorages, cifsStorages...)
...
func (k *kataAgent) createSMBVolumeObject(c *Container, m Mount) (*grpc.Storage, error) {
	...
	vol.Driver = kata9pDevType // 9p as of now calls a default storage handler in kata agent that does a baremount with these options
	return vol, nil
}
